...
Usporedba s otvorenim oceanom (Sjeverni Atlantik) gdje $H_S$ rutinski doseže 8--12 m s periodima $T_P = 12$--$18$ s jasno pokazuje ograničenja zatvorenih mora. Na @fig-spektri-stanja-mora prikazani su primjeri Pierson-Moskowitz spektara za tipična stanja mora.

```{python}
#| label: fig-spektri-stanja-mora
#| fig-cap: "Pierson-Moskowitz spektri valova za tipična stanja mora (SS 3--7)."

import numpy as np
import matplotlib.pyplot as plt

def pm_spectrum(omega, Hs, Tp):
    """Pierson-Moskowitz (ITTC) spektar valova."""
    wp = 2 * np.pi / Tp
    S = (5 / 16) * Hs**2 * wp**4 * omega**(-5) * np.exp(-5/4 * (omega / wp)**(-4))
    return S

omega = np.linspace(0.1, 2.5, 500)

sea_states = {
    "SS 3 ($H_S$=1.0 m, $T_P$=5.5 s)":   (1.0, 5.5),
    "SS 4 ($H_S$=2.0 m, $T_P$=7.5 s)":   (2.0, 7.5),
    "SS 5 ($H_S$=3.0 m, $T_P$=9.0 s)":   (3.0, 9.0),
    "SS 6 ($H_S$=5.0 m, $T_P$=12.0 s)":  (5.0, 12.0),
    "SS 7 ($H_S$=7.5 m, $T_P$=15.0 s)":  (7.5, 15.0),
}

fig, ax = plt.subplots(figsize=(8, 4.5))
for label, (Hs, Tp) in sea_states.items():
    S = pm_spectrum(omega, Hs, Tp)
    ax.plot(omega, S, label=label, linewidth=1.5)

ax.set_xlabel(r"Kružna frekvencija, $\omega$ (rad/s)")
ax.set_ylabel(r"Spektralna gustoća, $S(\omega)$ (m$^2$s)")
ax.set_xlim(0, 2.5)
...
Kao što je vidljivo na grafu, s porastom stanja mora (veći $H_S$ i $T_P$) spektar postaje širi i viši, što znači da more sadrži više energije raspoređene na nižim frekvencijama. Stanja mora SS 3--4 tipična su za Jadran pri umjerenim vjetrovima, dok SS 6--7 odgovaraju olujnim uvjetima na otvorenom oceanu.
