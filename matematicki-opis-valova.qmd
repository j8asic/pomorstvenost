...
(function() {
    ...
    // Physics solver for Wave Number (k) using Newton-Raphson
    function getWaveNumber(omega, d) {
        let k = (omega * omega) / g; // Deep water guess
        if (k * d < 1) {
            k = omega / Math.sqrt(g * d); // Shallow water guess
        }
        for(let i=0; i<10; i++) {
            const f = g * k * Math.tanh(k * d) - omega * omega;
            const df = g * Math.tanh(k * d) + g * k * d / Math.pow(Math.cosh(k * d), 2);
            k = k - f / df;
        }
        return k;
    }
    ...
    function draw() {
        ...
        // Draw Wave Surface (Lagrangian / Parametric approach)
        // This ensures the animated surface particle stays exactly on the free surface
        ctx.beginPath();
        const a_surf = amplitude * (Math.cosh(k * d) / Math.sinh(k * d));
        const b_surf = amplitude;
        
        let first = true;
        for(let px0 = -100; px0 <= width + 100; px0 += 5) {
            const mx0 = px0 / pixelsPerMeter;
            const xi_s = -a_surf * Math.sin(k * mx0 - omega * time);
            const zeta_s = b_surf * Math.cos(k * mx0 - omega * time);
            
            const px = px0 + xi_s * pixelsPerMeter;
            const py = zeroY - zeta_s * pixelsPerMeter;
...
![Područja primjenjivosti različitih valnih teorija (Le Méhautéov dijagram, izvor: Wikimedia Commons).](https://upload.wikimedia.org/wikipedia/commons/e/e0/Water_wave_theories.svg){#fig-valne-teorije width="70%"}
...
![Vizualni prikaz stanja mora prema WMO klasifikaciji s prosječnim značajnim visinama valova (izvor: Wikipedia).](images/sea-state-code-height.png){#fig-wmo-sea-state width="100%"}
